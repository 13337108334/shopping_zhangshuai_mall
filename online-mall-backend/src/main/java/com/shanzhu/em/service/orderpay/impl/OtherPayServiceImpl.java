...
public class OtherPayServiceImpl extends AbstractPayService {
    ...
    public ResultData<Order> payOrder(PayTypeEnum payTypeEnum, Long id) {
        ...
        if ("已支付".equals(resultData.getData().getState())) {
            logger.error("OtherPayServiceImpl getOrder order state 订单id为:{},status:{} desc:{}", JSON.toJSONString(id), JSON.toJSONString(resultData.getData().getState()),"该订单已支付 无需再次支付");
